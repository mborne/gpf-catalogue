...
says so on every page, in a banner placed ahead of every figure it qualifies rather than
in the footer: a catalogue overview is easy to mistake for the catalogue's own. The
banner is one line — the sentence that carries the warning — and it links to `/about`,
which is the rest: who published the site, that almost all of it was generated by
Claude, how the figures are produced and how stale they can be, where the authoritative
catalogue is, and the
[mentions légales](https://mborne.github.io/mentions-legales/), which the footer
repeats. The warning has to be read on every route; the paragraph explaining it has to
be read once, and six lines of it above every dashboard is how a notice stops being
read. The `<title>` carries the word too, on every route, since that is what a
...
| `/about` | What the site is, who published it, how the figures are built — the paragraph the banner used to carry. It is the one route that renders without `catalogue.json`, which is when it is most worth reaching |
